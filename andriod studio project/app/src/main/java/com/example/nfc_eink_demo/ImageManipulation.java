...
public class ImageManipulation {
	 ...
	 * @param src The original bitmap
     * @param reverseFlag whethere reverse the white/black color map
	 * @return Black and white bitmap
	 */
	public static Bitmap createBlackAndWhite(Bitmap src, int reverseFlag) {
		int width = src.getWidth();
		int height = src.getHeight();
		...
		for (int x = 0; x < width; ++x) {
			for (int y = 0; y < height; ++y) {
                ...
                //gray is the data send to tag
				int gray = (int) (0.2989 * R + 0.5870 * G + 0.1140 * B);
                //grayShow is the data shown on the phone (color is inversed)
                //int grayShow = (int) (0.2989 * R + 0.5870 * G + 0.1140 * B);
				// use 128 as threshold, above -> white, below -> black
				if (gray > 128) {
                    gray = 0xFF*(1-reverseFlag);
                    //grayShow = 0x00;
                }else {
                    gray = 0x00+(reverseFlag)*0xFF;
                    //grayShow = 0xFF;
                }
				// set new pixel color to output bitmap
				bmOut.setPixel(x, y, Color.argb(A, gray, gray, gray));
                //bmOut.setPixel(x, y, Color.argb(A, grayShow, grayShow, grayShow));
